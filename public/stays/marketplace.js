...
(async()=>{const root=document.querySelector('#featured-property-track');if(!root)return;
 ...
 try{const r=await fetch('/stays/property-index.json',{cache:'no-store'});const all=await r.json();const items=all.filter(p=>p.published&&p.featured&&p.cover_photo);if(!items.length){root.closest('.property-carousel-shell').hidden=true;return}
