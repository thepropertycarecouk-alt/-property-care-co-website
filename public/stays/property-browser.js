...
(async()=>{const grid=document.querySelector('#property-grid');if(!grid)return;const count=document.querySelector('#results-count');
 ...
 async function render(){const my=++seq,bed=Number(filters.bed.value||0),sleep=Number(filters.sleep.value||0),parking=filters.parking.value;
  ...
  if(selected){
    ...
    count.textContent='Choose a location from the suggestions to search within 15 miles.';
  }
  if(my!==seq)return;
  if(!filters.q.value.trim()||selected)count.textContent=listData.length+' '+(listData.length===1?'property':'properties')+(radiusUsed?' within 15 miles':'')+' displayed';
